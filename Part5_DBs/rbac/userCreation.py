...
for i in range(len(loadUsers)) :
  ...
  userpassword = loadUsers.loc[i, "pwd"] 
  ...
  ruleCopy = ruleCopy.replace("USERPASSWORD", userpassword)
...
open("./Part5_DBs/rbac/userCreation.sql", 'w').close()
#write the new rules to it
f= open("./Part5_DBs/rbac/userCreation.sql","w+")
